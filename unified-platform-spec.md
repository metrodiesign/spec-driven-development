...
- **Role gate ของ `RUN_COMMAND` ก่อน spawn (v1.10):** gate คือ `checkCommand(role)` ซึ่งเป็น required method บน interface `PathPolicy` (`core/src/executor/path-policy.ts`) · role ที่รันคำสั่งได้คือ `implementer` และ `diagnostician` เท่านั้น (`COMMAND_ROLES`) · role อื่นถูกปฏิเสธ **ก่อนถึงจุด spawn** ด้วย reason `command_role_denied` จึงไม่มี process ถูกสร้างเลย · gate นี้ทำงานเป็นด่านแรกในบล็อก `RUN_COMMAND` ของ executor **ก่อน network check และก่อน path allowlist จะมีความหมาย** — `WRITE_FILE` มี gate ตรวจสิทธิ์ *ก่อนเขียน* (`checkWrite`) อยู่แล้ว ส่วนสิ่งที่ถูกบังคับ *หลังรัน* ด้วย mutation diff เทียบ `writeRoots(role)` คือการเขียนที่เกิดจาก `RUN_COMMAND` เอง; role นอกสิทธิ์จึงถูกตัดที่ต้นทาง ไม่ใช่ปล่อยให้รันคำสั่งอ่าน/เผา CPU ได้ฟรี
...
| Test Designer/Property | codeProposal | Claude — **ต้องต่าง lineage กับ implementer** · เป้าหมาย default = **GLM-5.3** (open-weights coding SOTA — fit `codeProposal`; access มีแล้ว 2026-08-14, v1.11 ยกเลื่อน deferral — กลับมาเป็น default เมื่อ `openai-compatible.ts` ผ่าน conformance P1–P8 และลงทะเบียน, spec `.ai/specs/glm-5-3-adapter/`, ดู §7.6/§17) |
| Reviewer/Diagnostician | reasoning, largeContext | Claude + Codex ensemble · เพิ่ม **GLM-5.3** (context 1M tokens — fit `largeContext`) เมื่อ adapter พร้อมตามเงื่อนไขเดียวกับแถวบน (v1.11) |
...
`adapters/anthropic.ts` (Claude — primary, §5.2 — Phase 1, ส่งมอบแล้ว) · `adapters/codex.ts` (Codex — Phase 3; sandbox เป็น execution backend *ทางเลือก*) · `adapters/openai-compatible.ts` (GLM-5.3 + compat — **access มีแล้ว (2026-08-14 — GLM Coding Plan/ZCode + openai-compatible API) → v1.11 ยกเลื่อน deferral, spec งาน = `.ai/specs/glm-5-3-adapter/`**; ข้อมูลโมเดล: open-weights coding SOTA (+50% over GLM-5.2 บน Z.ai Code Bench), context 1M tokens / max output 128K, thinking เปิดเสมอ — API บังคับ `reasoning_effort` (low/high/max) เพราะ `thinking.type: "disabled"` ถูกถอน; aggregator เช่น OpenRouter = ผู้ประมวลผลข้อมูลอีกราย → `provider_data_policy` ต้องระบุ path ที่อนุญาต) · `adapters/_template.ts` — ทุกตัวบาง แปล wire format เท่านั้น
...
├── adapters/    # RING 2: anthropic.ts(primary — P1), codex.ts (P3), openai-compatible.ts (GLM-5.3 — v1.11 เปิดงานแล้ว: spec .ai/specs/glm-5-3-adapter/), _template.ts
...
- GLM-5.2 adapter (`adapters/openai-compatible.ts`) — **ยังไม่มี access (v1.3) → deferred ต่อแบบมีเงื่อนไข ไม่อยู่ใน scope Phase 4** (เลื่อนแบบบันทึกไว้ ไม่ตัดเงียบ); เมื่อมี access จริงค่อยเพิ่มตาม INV-8 (adapter 1 ตัว + conformance P1–P8, ห้ามแตะ Ring 0/1) — **อัปเดต v1.11: access มีแล้วจริง (GLM-5.3, 2026-08-14) → เงื่อนไขนี้สำเร็จแล้ว, งานเปิดที่ spec `.ai/specs/glm-5-3-adapter/`**
...
**จุดเริ่ม (v1.11):** Phase 0–4 ส่งมอบแล้ว; Phase 5 stage 1–4 ส่งมอบแล้ว; v1.10 = role allowlist gate สำหรับ `RUN_COMMAND` ก่อน spawn (executor อนุญาต implementer/diagnostician) + เปิด prompt vocabulary ให้ implementer เท่านั้น (ไม่ใช่ stage ใหม่ ไม่ขยับ roadmap เช่นเดียวกับ v1.8/v1.9 ก่อนหน้า) → งานถัดไป = **stage 5 (Evidence backflow) ตาม §14 ตามเดิม** · **GLM-5.3 adapter** (v1.11 — spec `.ai/specs/glm-5-3-adapter/`) · เมื่อพบความไม่ตรงกับพฤติกรรมจริงของ Claude Code/SDK: บันทึก `docs/DEVIATIONS.md` ตาม §0.6

**Changelog:**
- **v1.11** — **ยกเลื่อน deferral ของ GLM: GLM-5.2 → GLM-5.3 และเปิดงาน adapter จริง** (spec: `.ai/specs/glm-5-3-adapter/`) — ไม่ใช่ roadmap stage ใหม่ แต่เป็นการทำตามเงื่อนไขที่ v1.2/v1.3 บันทึกไว้ ("เมื่อมี access จริงค่อยเพิ่มตาม INV-8") · **เหตุ:** Z.ai เปิดตัว GLM-5.3 (2026-08-14) และ access มีแล้วจริง (GLM Coding Plan/ZCode — โมเดลรุ่นนี้รันอยู่จริงบนเครื่องนี้ผ่าน ZCode) เงื่อนไข deferral จึงเป็นจริงแล้ว · **ข้อมูลโมเดลที่ใช้ตัดสิน (แหล่ง: z.ai/blog/glm-5.3):** base model เดียวกับ GLM-5.2 — ทุก gain มาจาก post-training · open-weights coding SOTA (+50% over GLM-5.2 บน Z.ai Code Bench; Terminal-Bench 3.0 4.6→28.3; DeepSWE 46.2→66.9; SWE-Marathon 19.4→42.5) · context **1M tokens / max output 128K** (ตอบ capability `largeContext` ของ Reviewer/Diagnostician) · **API เปลี่ยน: `thinking.type: "disabled"` ถูกถอน — ต้องส่ง `reasoning_effort` (low/high/max) เสมอ** (กระทบ design ของ adapter โดยตรง) · **แก้ให้ตรงจริง:** §7.4 — Test Designer default เป้าหมาย = GLM-5.3 + Reviewer ensemble เพิ่ม GLM-5.3 **เมื่อ adapter ผ่าน conformance P1–P8 และลงทะเบียน** (จนถึงจุดนั้น default คง Claude/Codex ตาม v1.2/v1.3 — ไม่กลับก่อนเวลา), §7.6 — ลบ "deferred จนกว่าจะมี access" แล้วเขียนสถานะ+ข้อมูลโมเดลใหม่, §14 tree comment + clause หน้างานของ Phase 4 · **ไม่แตะ:** ประวัติส่งมอบ Phase 3/4 ตามที่เกิดจริง (คำว่า GLM-5.2 ในบริบท history คงเดิม), changelog v1.2/v1.3, archive specs, INV-7 (regex vendor-free คง `glm` — เป็นชื่อ lineage ไม่ใช่เวอร์ชันโมเดล)

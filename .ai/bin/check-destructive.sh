...
# ค้างทำให้ subcommand ไม่ match (bypass), และ `--flag[=val]` ทั่วไป. value-taking long-opt alt
...
GO='([[:space:]]+(-[cC][[:space:]]+[^[:space:];&|]+|--(git-dir|work-tree|namespace|super-prefix|exec-path|config-env|attr-source|object-format)[[:space:]]+[^[:space:];&|-][^[:space:];&|]*|--[^[:space:];&|]+))*'

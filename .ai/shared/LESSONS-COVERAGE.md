...
| slice-design-cell-heading-mismatch | mechanizable | not yet built — needs an operator decision first (authoring-convention change vs. looser matcher) before a check can be written; candidate check = lint every `.ai/specs/*/design.md` traceability cell1 against its own `## ` headings and fail below some match threshold |

...
- [x] 13. LIVE pass (manual — never CI; Phase-1 task-11 analog) — live Codex conformance P1-P8 (expect INV-16 wire-vocabulary class: fix Ring 2 prompt/classification, never verdicts; discrimination self-test stays green), §5.3 non-interactive-usage policy review recorded BEFORE first fusion activation, <=3 live fusion activations under profile caps -> uplift interval into `docs/calibration/`, real login + F-Loop approve from a second machine over tailnet (Basic; OIDC if Tailscale Serve available, else recorded fallback), one-time governance approval of the new policy snapshot, auditor real sample, real-repo auto-merge stretch (not DoD), replay/cache per-run and never committed. Done = §14 Phase 3 DoD evidence complete.
     Satisfies: REQ-2, REQ-3, REQ-11 (partial — mechanism proven live, uplift number not produced), REQ-19, REQ-20 (live re-verification — CI coverage lives in tasks 1-12). Depends on: 1-12. Verify: `docs/calibration/RUNBOOK-phase3.md` evidence + `bash scripts/spec-trace.sh platform-phase3`.
     Evidence:
       - §5.3 policy review: reviewed 2026-07-08 against the official Anthropic Help Center source (support.claude.com) BEFORE any fusion activation — the June 15 2026 non-interactive-usage billing change was paused, subscription quota still covers this platform's adapter calls. Recorded, not a permanent guarantee.
       - test: live Codex conformance P1-P8 — 5 real attempts, root-caused every FAIL, final record ALL PASS (`conformance-codex-2026-07-08T13-17-35-031Z.json`), P7 susceptibility 0. Root causes: OpenAI Structured Outputs "strict" mode (additionalProperties:false + full required-as-nullable + explicit array items, none of which Anthropic requires) was undocumented in this repo until this task; fixed in `aal/src/conformance/harness.ts` (real actionRequests.items shape + echo field), `aal/src/repair.ts` (`validateAgainstSchema` accepts a union-array `type`), `adapters/src/codex-live.ts` (new `strictifyForCodex`, Codex-only), `adapters/src/codex.ts` (surface a parsed `--json` error event over bare stderr) — 565/565 pre-existing tests stayed green throughout, 6/6 packages typecheck clean.
       - test: live fusion — 3 activations (self-panel `hypotheses`, 2x cross-vendor `plan` Claude+Codex), all real winners, no escalation, 67.776 costUnits total (cap ~3 activations, decision #3). Exercised the real `fusion.deliberate` seam directly (real Registry/Router/Dispatcher/`runFusion`, no FakeAdapter) rather than building the undocumented "planner-role auto-routing" trigger task 3's deviation note assumed existed — that trigger has zero implementing code anywhere in the tree (recorded as backlog, not silently dropped).
       - test: live loop -> real auto-merge -> real auditor sample. `.ai/calibration/fixture-goal-l0.yaml` (new, `risk: L0`) routes through `decideAutoApprove`'s automatic path (no human step). 4 live attempts (3 BLOCKED on model tool-request variance, 1 COMPLETED) -> real `AUDIT_RESULT` -> `platform auditor run --db ... --repo ...` -> `T-1 reproduced`, original/rerun gate-evidence hashes byte-identical. Found+fixed in passing: `platform loop run`'s fixture repo was unconditionally deleted regardless of `--live`/persistDir, so no run ever left an auditable repo behind — `console/backend/src/loop-run.ts` now clones the post-merge worktree into `<persistDir>/repo` when persisting, verified free (FakeAdapter, in-process) before spending live quota on it.
       - governance: one real `platform governance approve` after the `fusion-profiles.json` self-diversity addition (hypotheses artifact switched from cross_lineage to self, seeds `[7,13]`) — `.ai/governance/events.jsonl` carries the real GOVERNANCE_PROPOSED + GOVERNANCE_CHANGE pair.
       - CLI wiring added this task (previously never connected to anything): `platform conformance --live --lineage claude|codex` (was hardcoded to Claude only); `latestConformanceRecordPath(dir, adapterId?)` now filters by lineage (was picking the lexicographically-latest record regardless of which adapter was about to be constructed — a live, real mismatch caught before it could register the wrong adapter under the wrong id).
       - viewports: n/a — no new UI this task.
       - deviations: (1) **F-Loop approve from a second machine: NOT completed.** Tailscale connected cleanly (this machine joined the tailnet; a second device never became available this session) but the real blocker is one layer under the network — investigated and confirmed the REVIEWING -> human-approval -> merge-continuation pipeline was never wired end-to-end in production despite tasks 5/6/8 shipping `[x]`: the Human Plane server's `approvals` Map is permanently empty in every real CLI run (`buildApprovalPackage` has exactly two callers repo-wide, both test files), and even a reachable approval would dead-end (`runAutoMerge` already ran automatically before any human step; a human decision today only appends `TASK_STATE:APPROVED` and nothing continues it). This is a gap task 13 discovered, not one it caused — task 8's own browser-verify evidence proved the HTTP/UI layer correct against a hand-seeded Map, not that a real run ever produces one. Recorded in `docs/calibration/RUNBOOK-phase3.md` step 4, carried to backlog as its own follow-up bugfix (populate the Map on an auto-merge decline + continue the pipeline post-approval) rather than improvised live — the same category of decision as (2) below, made explicitly with the user rather than silently. (2) **Fusion uplift number: not produced.** The 3 live activations (self-panel + 2 cross-vendor) prove the fusion mechanism for real but used `plan`/`hypotheses` artifacts (judge-pick / mechanical resolve, no `GateReport`) — a genuine `computeFusionCalibration` interval needs a `code_diff`/`tests` activation (real gate evidence) plus a single-model baseline on the same calibration-corpus task, roughly double this task's already-spent fusion budget; the user chose to record this rather than spend further. (3) The design's "policy trigger" fusion entry point (`loop-run.ts` auto-routing a live planner-role proposal through `runFusion`) has zero implementing code anywhere in the tree — task 3's deviation note describing it as "the live-only trigger exercised in task 13" was aspirational; the agent-invoked `fusion.deliberate` seam is what's real, and is what this task exercised. (4) real-repo auto-merge (stretch, explicitly not DoD) not attempted, superseded by the fixture-based L0 auto-merge proof above, which reaches the same real code path (`decideAutoApprove` -> direct `git merge --no-ff`) without touching this actual repo's own history. (5) Two Phase-1-carried residuals stay open unchanged (repair-round token usage uncounted against budget; frozen `nowMs` never trips the wallclock budget) — not in this task's scope, re-confirmed still true.

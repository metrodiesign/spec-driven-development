...
- 3.4 THE SYSTEM SHALL verify P4 by setting budget to 1 costUnit and requiring a degraded-but-
      valid response with no crash.
...
- 3.6 THE SYSTEM SHALL verify P6 by asserting zero tool_use blocks in the in-memory message
      stream and no claimed execution output (transcript as corroboration when available).
...
- 4.5 THE SYSTEM SHALL attempt transcript capture after each query: derive the JSONL path from
      the returned session id, poll briefly for flush, copy into the evidence store as
      rawTranscriptRef.
...
- 5.3 THE SYSTEM SHALL persist a durable replay record keyed by requestId at first completion;
      a repeated send with the same requestId SHALL be served from that record with usage
      counted once.
...
**Acceptance Criteria (EARS):**
- 6.1 THE SYSTEM SHALL select an adapter by capability match over the registered set.
- 6.2 IF no eligible adapter exists THEN THE SYSTEM SHALL surface structured `no_capacity` and
      the task SHALL become BLOCKED(no_capacity) with no retry loop.
...
- 7.3 WHEN GOVERN detects a secret-shaped string (core-own generic patterns + entropy heuristic,
      ...
      the file — never redact-and-send.
...
- 9.1 WHEN a task reaches REVIEWING THE SYSTEM SHALL generate an ApprovalPackage containing goal
      ...
      risks, and an attestation checklist generated from the risk class.
...
- 10.4 WHEN `POST /kill` is received THE SYSTEM SHALL let the current atomic action complete,
       ...
       the run, and record KILL_REQUESTED (kill ≠ pause).
...
- 11.3 WHEN `--live` is requested THE SYSTEM SHALL refuse if the `CI` environment variable is
       set or stdin is not a TTY, and SHALL print the costUnits cap and require confirmation
...
- 13.3 THE SYSTEM SHALL gate WS attachment with a single-use short-TTL ticket; reuse or expiry
       SHALL close with 4403/4401.
...
- 16.3 THE SYSTEM SHALL provide setup-token guidance text and SHALL expose no route that
       accepts, returns, or stores a token or credential (negative guarantee with tests).
...
- 19.1 THE SYSTEM SHALL install and uninstall the activity HTTP hooks in one click each; the
       installed hook entries SHALL be fail-open with a short timeout.
...
- Live-run confirmation (REQ-11.3) under AFK delegation: the confirmation is answered by the
  operator persona driving the terminal; the structural guards (CI env, TTY) stay absolute.

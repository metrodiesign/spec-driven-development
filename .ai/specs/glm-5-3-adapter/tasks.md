...
- [ ] 2. Conformance gate in CI — `adapters/src/openai-compatible-conformance.test.ts`: compliant fake transport passes P1–P8 via `runConformanceSuite`, prose-only saboteur fails exactly P2, regressed re-run marks the registered adapter `stale` through the registry gate.
